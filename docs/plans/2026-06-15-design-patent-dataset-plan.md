...
| 维度 | 标准 | 对生成的影响 |
|------|------|-------------|
| 产品种类 | 相同或相近用途 | 正负样本必须同品类 |
| 整体视觉效果 | 整体观察、综合判断 | 不以像素级比对，以整体印象为准 |
| 区别设计特征 | 创新点是否被复制 | 正样本保留核心特征 |
| 设计空间 | 该品类设计自由度 | 自由度大的品类，细微差别可忽略 |

**美国 Ordinary Observer Test**（Egyptian Goddess v. Swisa, 2008）：
以普通消费者视角判断是否误认为同一来源。
...
| 样本 | 条件 | 相似度 |
|------|------|--------|
| **正样本（侵权）** | 同类 + 整体相似 + 核心设计特征保留 | 50-75% |
| **负样本（不侵权）** | 同类 + 整体显著不同 | ≤ 75% |

### 典型案例

- 侵权：Apple v. Samsung（圆角矩形+Home键）、戴森吸尘器（整体造型）
- 不侵权：不同类产品（水杯 vs 台灯）、核心特征不同（同为保温杯但杯盖设计完全不同）
