...
> **法律依据**：参见 [LEGAL.md](LEGAL.md) — 外观设计专利侵权判定的法律标准与操作定义

## 功能需求

### F0: 侵权判定标准（法律 → 操作转换）

**F0.1** 正样本（侵权）SHALL 满足：同类产品 + 整体相似 + 区别设计特征保留。

**F0.2** 负样本（不侵权）SHALL 满足：同类产品 + 整体显著不同（与任意注册图相似度 ≤ 75%）。

**F0.3** 相似度阈值 SHALL 基于 CNIPA"整体视觉效果"标准及 US"Ordinary Observer Test"：
- 正样本与注册图相似度：50-75%（保留核心特征，改变非核心细节）
- 负样本与任意注册图相似度：≤ 75%（一般消费者可区分）
